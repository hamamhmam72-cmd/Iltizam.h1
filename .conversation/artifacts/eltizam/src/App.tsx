  ...
  return (
    <header className="public-header">
      ...
      <div className="public-header-actions">
        <span className="live-dot"><i /> الخدمة تعمل الآن</span>
      ...
      <main className="public-main">
        <section className="public-hero">
          <div className="hero-copy">
            ...
            <div className="trust-row"><span><ShieldCheck size={17} /> كباتن موثوقون</span><span><Clock3 size={17} /> متابعة مباشرة</span></div>
          </div>
          <div className="request-panel">
            {!created ? (
              <form onSubmit={submit}>
                ...
                <small className="form-footnote">بإرسال الطلب، يوافق العميل على سياسة الخدمة والخصوصية.</small>
              </form>
            ) : (
...
function Login() {
  ...
  return (
    <div className="login-page" dir="rtl">
      <div className="login-side"><Brand /><div className="login-side-copy"><span className="section-kicker">غرفة العمليات</span><h1>القرار الصحيح،<br /><em>في الوقت الصحيح.</em></h1><p>منصة التزام تجمع حركة الطلبات، الكباتن، والذمم المالية في شاشة واحدة.</p></div><div className="login-side-footer">بياناتك محفوظة ومشفرة دائماً</div></div>
      <div className="login-card-wrap"><Link href="/" className="back-home" data-testid="link-back-home"><ArrowLeft size={16} /> العودة للواجهة العامة</Link><div className="login-card"><div className="login-card-heading"><span className="section-kicker">دخول آمن</span><h2>أهلاً بعودتك</h2><p>اختر مساحتك للوصول إلى لوحة العمل.</p></div><div className="role-switch"><button className={role === 'admin' ? 'selected' : ''} onClick={() => setRole('admin')} data-testid="button-role-admin"><ShieldCheck size={18} /><span>مدير العمليات<small>إدارة ومتابعة</small></span></button><button className={role === 'captain' ? 'selected' : ''} onClick={() => setRole('captain')} data-testid="button-role-captain"><Truck size={18} /><span>كابتن<small>طلباتي وذممتي</small></span></button></div><form onSubmit={login}><Field label="الاسم أو رقم الجوال" value={name} onChange={setName} placeholder="اكتب اسمك للمتابعة" icon={UserRound} required={false} testId="input-login-name" /><button className="button button-primary button-wide" type="submit" data-testid="button-login">متابعة إلى اللوحة <ArrowLeft size={17} /></button></form><p className="login-demo">وضع العرض التجريبي — لا يتطلب كلمة مرور</p></div></div>
    </div>
  );
}
...
function AdminRoute() {
  ...
  return allowed ? <AppShell role="admin"><Switch><Route path="/admin" component={AdminOverview} /><Route path="/admin/orders" component={AdminOrders} /><Route path="/admin/finance" component={AdminFinance} /><Route component={NotFound} /></Switch></AppShell> : null;
}

function CaptainRoute() {
  const allowed = useRoleGuard('captain');
  return allowed ? <AppShell role="captain"><Switch><Route path="/captain" component={CaptainDashboard} /><Route path="/captain/orders" component={CaptainOrders} /><Route component={NotFound} /></Switch></AppShell> : null;
}

function Router() {
  return <ErrorBoundary><Switch><Route path="/" component={PublicHome} /><Route path="/login" component={Login} /><Route path="/admin/*" component={AdminRoute} /><Route path="/captain/*" component={CaptainRoute} /><Route component={NotFound} /></Switch></ErrorBoundary>;
}
